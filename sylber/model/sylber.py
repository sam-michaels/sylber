    ...
    def __call__(self, wav_file=None, wav=None, in_second=True):
        ...
        for i, segments in enumerate(all_segments):
            ...
            result = {
                ...
                'segment_features': np.stack([states[s:e].mean(0) for s, e in segments]) if len(
                    segments) > 0 else np.array([]),
...
class Sylber(nn.Module):
    ...
    def __init__(self,
        ...
        if load_hubert:
            ...
            self.speech_model = HubertModel(
                HubertConfig.from_pretrained(speech_upstream, num_hidden_layers=encoding_layer))
    ...
    def segment(self, input_values=None, features=None, attention_mask=None, mergethreshold=None, normthreshold=None,
                **kwargs):
        """
        """
        if features is None:
        ...
        if segments is None:
            ...
            with torch.no_grad():
                ...
                if self.merge_threshold_range[0] < self.merge_threshold_range[1]:
                    ...
                    merge_threshold = self.merge_threshold_range[0]
                segments = [get_segment(states.cpu().numpy(), normthreshold, merge_threshold) for states in
